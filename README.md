# Make a Rootstock NFT Collection With Foundry

## Requirements
Metamask (or other supported wallet)
Test RBTC from this faucet
Foundry
Pinata account
...
```
Note that the safeMint function takes in the hash of a file already uploaded to a storage service. For this tutorial, we will use Pinata. This step will come later, for now proceed with any valid string.
...
We'll need actual files with valid token URIs for this.
Create an account on [Pinata](app.pinata.cloud) and go to the [files section](https://app.pinata.cloud/ipfs/files) .
Create your nft metadata. Upload an image and note the hash
